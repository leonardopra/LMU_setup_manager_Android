...
2. **`SetupViewModel` default car**: hardcodes `"bmw-m4-gt3"` in the `MutableStateFlow` initializer. `initForCar()` must always be called from `SetupScreen`; if skipped, BMW defaults silently apply.
